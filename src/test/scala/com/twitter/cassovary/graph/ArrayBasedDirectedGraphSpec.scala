...
case class DeepEqualsNode(a: Node) extends Matcher[Node]() {
  def apply(b: => Node) = (
    (a.id == b.id) && (a.outboundNodes.toList == b.outboundNodes.toList) &&
        (a.inboundNodes.toList == b.inboundNodes.toList),
      "Mutable nodes are equal",
      "Mutable nodes did not match. Expected: %s Actual: %s".format(a, b)
  )
}

case class DeepEqualsNodeIterable(aIterable: Iterable[Node]) extends Matcher[Iterable[Node]]() {
  def apply(bIterable: => Iterable[Node]) = (
    {
      val aList = aIterable.iterator.toList
      val bList = bIterable.iterator.toList
      (aList.size == bList.size) &&
        (0 until aList.size).foldLeft(true) { (truth, index) =>
          truth &&
          { val a = aList(index)
            val b = bList(index)
            (a.id == b.id) && (a.outboundNodes.toList == b.outboundNodes.toList) &&
    ...
    "Node iterators are equal",
    "Node iterators did not match. Expected: %s Actual: %s".format(aIterable, bIterable)
  )
}


object NodeMaker {
  def apply(id: Int, in: Array[Int]): Node = {
    apply(id, Array(), in)
  }

  def apply(nodeId: Int, out: Array[Int], in: Array[Int]): Node = {
    apply(nodeId, out, in, StoredGraphDir.BothInOut)
  }

  def apply(nodeId: Int, out: Array[Int], in: Array[Int], storedGraphDir: StoredGraphDir): Node = {
...
class ArrayBasedDirectedGraphSpec extends Specification {
  ...
  def makeGraph(dir: StoredGraphDir.StoredGraphDir) = ArrayBasedDirectedGraph(iteratorFunc, dir)
