...
object ArrayBasedDirectedGraphWithSimulatedCache {
   ...
   */
  def apply(iteratorSeq: Seq[ () => Iterator[NodeIdEdgesMaxId] ], executorService: ExecutorService,
            storedGraphDir: StoredGraphDir, cacheSize: Int, cacheMechanism: String) = {
    ...
    new ArrayBasedDirectedGraphWithSimulatedCache(table.asInstanceOf[Array[Node]], maxNodeId,
      nodeWithOutEdgesMaxId, nodeWithOutEdgesCount, numNodes, numEdges, storedGraphDir, cacheSize, cacheMechanism)
  }
  @VisibleForTesting
  def apply( iteratorFunc: () => Iterator[NodeIdEdgesMaxId],
...
class ArrayBasedDirectedGraphWithSimulatedCache (nodes: Array[Node], maxId: Int,
    val nodeWithOutEdgesMaxId: Int,
    val nodeWithOutEdgesCount: Int, val nodeCount: Int, val edgeCount: Long,
    val storedGraphDir: StoredGraphDir, val cacheSize: Int = 10, val cacheMechanism: String) extends DirectedGraph {
