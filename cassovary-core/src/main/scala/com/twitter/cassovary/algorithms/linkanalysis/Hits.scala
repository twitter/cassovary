...
/**
 * Class for performing Hits algorithm.  Hits is a link analysis algorithm that returns two values characterizing
...
class Hits(graph: DirectedGraph[Node], params: HitsParams = HitsParams())
  ...
  private def collect(node: Node, targetCollection: Array[Double],
                      valueCollection: Array[Double], scatter: Boolean): Unit = {
  ...
  protected def defaultInitialState: HitsIterationState = {
    val hubs = Array.tabulate(graph.maxNodeId + 1){ n => if (graph.existsNodeId(n)) 1.0 / graph.nodeCount else 0.0 }
  ...
  def iterate(prevIteration: HitsIterationState): HitsIterationState = {
    ...
    val partialAuth = new Array[Double](graph.maxNodeId + 1)
    val partialHubs = new Array[Double](graph.maxNodeId + 1)
     ...
     */
    def authorities(node: Node) = collect(node, partialAuth, beforeHubs, scatter = !isInStored)
     ...
     */
    def hubs(node: Node) = collect(node, partialHubs, partialAuth, scatter = isInStored)
