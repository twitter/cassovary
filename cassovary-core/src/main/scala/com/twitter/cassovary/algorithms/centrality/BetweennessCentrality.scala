...
class BetweennessCentrality(graph: DirectedGraph[Node], normalize: Boolean = true) extends AbstractCentrality(graph) {
  ...
  def _recalc(): Unit = {
    val partial = graph.par map { n =>
      val allPaths = new SingleSourceShortestPath(graph, n.id).allShortestPaths
      val currentValues = allPaths.par.values map { pc =>
        pc.view.flatMap { p => p.slice(1, p.size - 1) }
          .groupBy { k => k }
          .map { k => k._1 -> (k._2.size.toDouble / pc.length) }
      }
      currentValues reduce combineMap
    }

    val complete = partial reduce combineMap

    val nc = graph.nodeCount
    val scale = if (normalize && nc >= 2) 1.0 / ((nc - 1) * (nc - 2)) else 1.0
    complete.par foreach { case (i, v) => centralityValues(i) = v * scale }
  }
}
