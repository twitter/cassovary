...
private[collections] class MappedOrder[@specialized A, @specialized B](order: Order[B])(f: A => B) extends Order[A] {
  def compare(x: A, y: A): Int = order.compare(f(x), f(y))
}

private[collections] class ReversedOrder[@specialized A](order: Order[A]) extends Order[A] {
...
object Order {
  ...
  def by[@specialized A, @specialized B](f: A => B)(implicit o: Order[B]): Order[A] = o.on(f)

  def from[@specialized A](f: (A, A) => Int): Order[A] = new Order[A] {
