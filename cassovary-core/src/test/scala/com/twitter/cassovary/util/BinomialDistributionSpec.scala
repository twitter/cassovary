...
class BinomialDistributionSpec extends WordSpec with ShouldMatchers {
  ...
  "two point distribution" should {
    "have correct pdf" in {
      twoPoint.pdf(0) should be (0.7)
      twoPoint.pdf(1) should be (0.3)
    }
    "have correct cdf" in {
      twoPoint.cdf(0) should be (0.7)
    }
  }
  ...
  "binomial distribution with 2 trials" should {
    ...
    "have correct pdf" in {
      twoTrials.pdf.zip(expected).foreach{case (x, y) => x should be (y plusOrMinus 0.001)}
    }

    "have correct cdf" in {
      val expected = Array(0.2 * 0.2, 0.2 * 0.2 + 2 * 0.2 * 0.8)
      twoTrials.cdf.zip(expected).foreach{case (x, y) => x should be (y plusOrMinus 0.001)}
    }
  }

  "binomial distribution with 10 trials" should {
    "have non-negative pdf" in {
      distribution10.pdf.foreach{
        p => p should be >= 0.0
      }
    }

    "have pdf that sums up to 1" in {
      distribution10.pdf.sum should be (1.0 plusOrMinus 0.005)
    }

    "have increasing cdf" in {
      distribution10.cdf.foldLeft(0.0){case (a, b) =>
        a should be < b
        b
      }
    }
    ...
    "give samples that follow pdf in terms of mse" in {
      ...
      val rng = new Random
      val histogram = distribution10.sample(sampleSize, rng)
        .groupBy(identity)
        .map{ case (x, y) => (x, y.length.toDouble / sampleSize)}
      var mse = 0.0
      for (i <- 0 to 10) {
        val error = distribution10.pdf(i) - histogram.getOrElse(i, 0.0)
        mse += error * error
      }
